    ...
    return (
        <div className="p-4 pt-6 flex flex-col justify-between max-w-80 shadow border border-gray-200 rounded-md bg-white space-y-2">
            ...
            <div className="flex  gap-2">
                <button onClick={handleFollow} disabled={currentUser.following.includes(user._id)} className="flex items-center justify-center bg-slate-200 hover:bg-slate-300 flex-1 rounded-md py-3 gap-1 active:scale-95 transition cursor-pointer bg-gradient-to-l from-indigo-500 to-purple-600">
                    <UserPlus className="w-4 h-4 text-white"/>
                    <span className=" text-white">{currentUser.following.includes(user._id) ? "Following" :"Follow"}</span>
                </button>
